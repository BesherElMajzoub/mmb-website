...
export default function SeriesDetail() {
  const [match, params] = useRoute('/products/:category/:seriesSlug');

  if (!match) return <NotFound />;

  const category = products.find(c => c.id === params.category);
  const series = category?.series.find(s => s.slug === params.seriesSlug);

  if (!category || !series) return <NotFound />;
      ...
      <div className="bg-muted/30 border-b border-border">
        <div className="container py-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <Link href={`/products/${category.id}`}><a className="hover:text-primary">{category.name}</a></Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="flex flex-wrap gap-4">
                <Button className="bg-primary hover:bg-primary/90 text-white font-bold uppercase tracking-wide rounded-none h-12 px-6 gap-2">
                  <FileText className="w-4 h-4" /> Read Datasheet
                </Button>
                <Button variant="outline" className="border-border hover:bg-muted font-bold uppercase tracking-wide rounded-none h-12 px-6 gap-2">
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-muted text-foreground font-heading uppercase tracking-wider text-xs">
                    <tr>
                      ...
                      <th className="px-6 py-4 font-bold border-b border-border">Unit</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/50">
                    {series.specs.map((spec, idx) => (
                      <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-muted/10'}>
                        ...
                        <td className="px-6 py-4 text-muted-foreground">{spec.unit}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <section className="py-16 bg-secondary text-secondary-foreground">
        <div className="container text-center">
          ...
          <div className="flex justify-center gap-4">
            <Button className="bg-primary hover:bg-primary/90 text-white font-bold uppercase tracking-wide rounded-none h-14 px-8 text-base">
